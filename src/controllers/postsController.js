...
const listPosts = async (req, res) => {
  ...
  const posts = await postService.listPosts(env.defaultUserId, { status, take, skip });
  return ok(res, { posts: posts.map(postService.toPublicShape) });
};

// GET /api/posts/:id
const getPost = async (req, res) => {
  const post = await postService.getPost(req.params.id, env.defaultUserId);
  return ok(res, { post: postService.toPublicShape(post) });
};
...
const updatePost = async (req, res) => {
  const post = await postService.updatePost(req.params.id, env.defaultUserId, req.body || {});
  return ok(res, { post: postService.toPublicShape(post) });
};

// POST /api/posts/:id/archive
const archivePost = async (req, res) => {
  const post = await postService.archivePost(req.params.id, env.defaultUserId);
  return ok(res, { post: postService.toPublicShape(post) });
};

// DELETE /api/posts/:id  (only for DRAFT; other states must be archived)
const deletePost = async (req, res) => {
  const result = await postService.deletePost(req.params.id, env.defaultUserId);
